...
python job.py <TRIAL_NUM> <FILENAME>
python jobs.py 1 13
"""
...
def exec_exp(nodes, mode_region, exp_type, code_script):
    ...
    for row in nodes:
        ...
        with open(log_dir_path + filename + '.csv', 'a+') as csvwritefile:
            ...
            writer.writerow({'trial#': trial_num, 'datestamp': datetime.now(), 'experiment type': exp_type
...
def main():
    ...
    if num_args != 3:
        print "USAGE: python run_experiments.py <TRIAL_NUM> <FILENAME>"
        return
    trial_num = sys.argv[1]
    print 'trial #:' +  trial_num
    log_dir_path = '/home/ubuntu/logs/' + trial_num + '/parallel/'
    filename = sys.argv[2]
