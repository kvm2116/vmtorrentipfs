...
def main():
    ...
    if num_args != 7:
        print "USAGE: python get_ipfs2.py <SRC_DIR> <DST_DIR> <TRIAL_NUM> <FILE_TYPE> <EXP_TYPE> <SCRIPT_TYPE>"
        return
    ...
    image_split = src_directory.rsplit('/',1)
    image_num = image_split[1]
            ...
            if private_ip == ip:
                if 'SHUFFLE' in exp_type:           # in SHUFFLE need to pull 2 files from other servers based on csv file
                    ...
                    for line in nodes:
                        ...
                        csvtemp.append(image_num)
                        csvtemp.append(line["privateip"])
                        csvtemp.append(now.strftime("%Y-%m-%d %H:%M:%S.%f"))
                        pub_ip = line["publicip"]
                        get_files(pub_ip, src_directory, dst_directory, csvtemp, image_num, private_ip, log_dir_path, script_type)
                        
                # elif 'UNDER_ATTACK' in exp_type:        # in UNDER_ATACK need to push 4 files to other servers based on csv file
                #     get_image1 = row["GETIPFSIMAGE1"]
                #     get_image2 = row["GETIPFSIMAGE2"]
                #     get_image3 = row["GETIPFSIMAGE3"]
                #     get_image4 = row["GETIPFSIMAGE4"]
                ...
                #                 elif (get_image == line["ipfsimage2"]):
                #                     csvtemp.append(line["nodename"])
                ...
                #                     pub_ip = line["publicip"]
                #                     get_files(pub_ip, src_directory, dst_directory, csvtemp, get_image, private_ip, log_dir_path, script_type)
                #                 elif (get_image == line["ipfsimage3"]):
                #                     csvtemp.append(line["nodename"])
                ...
                #                     pub_ip = line["publicip"]
                #                     get_files(pub_ip, src_directory, dst_directory, csvtemp, get_image, private_ip, log_dir_path, script_type)
                #                 elif (get_image == line["ipfsimage4"]):
                #                     csvtemp.append(line["nodename"])
...
def get_files(ip, src_directory, dst_directory, csvtemp, image, private_ip, log_dir_path, script_type):
    ...
    with open(log_dir_path + exp_type + script_type +
        ...
        fieldnames = ['trial#','from nodename', 'image','from ip',
                      'datestamp','start time', 'end time']
        writer = csv.DictWriter(csvwritefile, delimiter=',',
                                lineterminator='\n', fieldnames=fieldnames)
